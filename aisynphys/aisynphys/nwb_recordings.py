...
def get_intrinsic_recording_dict(nwb, dev_id):
...
def qc_recordings(expt, rec_list):
    for rec in rec_list:
...
def get_pulse_times(recording):
    ...
    # wave itself to find the stimlus
        ...
        # NWB 2.x oodDAQ: holding + epoch_baseline + epoch_pulse + epoch_baseline (4 items)
        # or similar layouts — find the SquarePulse with the largest absolute amplitude
        from neuroanalysis import stimuli as _stim_mod
        best = None
        for item in stims:
            if isinstance(item, _stim_mod.SquarePulse) and abs(getattr(item, 'amplitude', 0)) > 0:
                if best is None or abs(item.amplitude) > abs(best.amplitude):
                    best = item
        if best is not None:
            return best.start_time, best.start_time + best.duration
        return None
